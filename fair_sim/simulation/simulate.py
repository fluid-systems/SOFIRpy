...
class System:
    ...
    def __init__(self, simulation_entity: Union[Fmu, Controller], name: str) -> None:
        """Initialize System object

        Args:
            simulation_entity (Union[Fmu, Controller]): fmu or controller
...
class Controller(ABC):
    """Abstract object representing a controller."""
    ...
    def generate_output(self):
        """Performs controller calculation and sets parameters accordingly.
        """
        pass
...
class Simulation:
    ...
    def __init__(self, systems: list[System], connections: list[Connection], parameters_to_record: Optional[list[SystemParameter]] = None) -> None:
    ...
    def simulate(self, stop_time: float, step_size: float, start_time: float = 0.0) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: results dataframe with times series of logged parameters
        """
    ...
    def do_step(self, time: float, step_size: float) -> None:
        ...
        for system in self.systems:
            if isinstance(system.simulation_entity, Fmu):
                ...
                system.simulation_entity.generate_output()
    ...
    def get_units(self) -> dict[str, Union[str, None]]:
        ...
        If the unit of a parameter can not be obtained the unit is set to None.

        Returns:
            dict[str, str]: keys: parameter name, values: unit
        """ 
...
def simulate( stop_time: Union[float,int], step_size: float, 
                ...
                control_classes: Optional[dict[str, Controller]] = None,
                parameters_to_log: Optional[dict[str, list[str]]] = None, 
                get_units: Optional[bool]=False) -> Union[pd.DataFrame, tuple[pd.DataFrame, dict[str,str]]]:
    ...
    Args:
        ...
        fmu_infos (Optional[list[dict[str, Union[str, list[dict[str, str]]]]]], optional): 
            ...
            Note: The name of the fmus can be chosen arbitrarily, but each name in the controllers and the fmus must occur only once.
            Defaults to None.
        control_infos (Optional[list[dict[str, Union[str, list[dict[str, str]]]]]], optional): 
            Defines which controllers should be simulated and how they are connected to other systems. It needs to have the same format as 'fmu_infos' with the difference that
        ...
        parameters_to_log (Optional[dict[str, list[str]]], optional): Dictionary that defines which parameters should be logged. It needs to have the following format:
            >>> parameters_to_log = {
                    <name of system 1 (corresponding to the names specified in 'control_infos' or 'fmu_infos')>: 
                    [
                        <name of parameter 1>,
                        <name of parameter 2>,...
                    ], 
    ...
    Returns:
        Union[pd.DataFrame, tuple[pd.DataFrame, dict[str,str]]]: result Dataframe with times series of logged parameters, units of logged parameters
    """
    ...
    if fmu_infos and control_infos is None:
        raise ValueError(f"'fmu_infos' and 'control_infos' are empty; expected al least one to be not empty")
    ...
    systems = init_systems(fmu_infos, control_infos, control_classes)

...
class Attribute:
    ...
    def to_hdf5(self, hdf5: HDF5, parent: HDF5Entity) -> None:
        hdf5.append_attributes(self.attributes, parent.path)


@dataclass
class RunGroup(Group):
    run_name: str
    meta: Optional[RunAttr] = None
    models: Optional[ModelsGroup] = None
    simulation: Optional[SimulationResultsGroup] = None
    ...
    def from_config(
        ...
        self = cls()
        meta = RunAttr.from_config(config, self)
        models = ModelsGroup.from_config(config, self, model_instances=model_instances)
        results = SimulationResultsGroup.from_config(config, self)
        return cls(run_name=run_name, meta=meta, models=models, results=results)

    @classmethod
    def from_hdf5(cls, hdf5_path: Path, run_name: str) -> Self:
        hdf5 = HDF5(hdf5_path)
        self = cls(run_name)
        meta = RunAttr.from_hdf5(hdf5, self)
        self.meta = meta
        models = ModelsGroup.from_hdf5(hdf5, self)
        self.models = models
        simulation = SimulationResultsGroup.from_hdf5(hdf5, self)
        self.simulation = simulation
        return self

    def simulate(self) -> None:

        _results = simulate(**self.simulation.attr.attributes, **self.models.to_dict())
        if isinstance(_results, tuple):
            results, units = _results
            self.simulation.time_series.data = results
            self.simulation.time_series.attr.attributes = units
            return
        self.simulation.time_series.data = results

    def to_hdf5(self, hdf5_path: Path) -> None:
        assert self.simulation.time_series is not None
        hdf5 = HDF5(hdf5_path)
        ...
        try:
            ...
            self.meta.to_hdf5(hdf5, self)
            self.models.to_hdf5(hdf5, self)
            self.simulation.to_hdf5(hdf5, self)
...
class FmusGroup(Group):
    ...
    def from_config(
        ...
        fmu_groups = []
        for fmu_name, fmu_config in fmus_config:
    ...
    def from_hdf5(cls, hdf5: HDF5, parent: ModelsGroup) -> Self:
        self = cls(parent)
        fmu_groups = []
        for group_name in hdf5.get_group_names(parent.path):
...
class PythonModelsGroup(Group):
    ...
    def from_config(
        ...
        python_model_groups = []
        for python_model_name, python_model_config in python_models_config:
...
class ModelDataset(Dataset):
    ...
    def to_hdf5(self, hdf5: HDF5) -> None:
        hdf5.store_data(
            self.data, data_name=self.DATASET_NAME, group_path=self.parent.path
        )
...
class ParametersToLogDataset(ModelDataset):
    ...
    DATESET_NAME = "parameters_to_log"
...
class FmuReferenceDataset(ModelDataset):
    data: str
    DATESET_NAME = "reference"

    @classmethod
    def from_hdf5(cls, hdf5: HDF5, parent: FmuGroup) -> Self:
        data = hdf5.read_data(cls.DATASET_NAME, parent.path)
...
class ModelGroup(Group):
    ...
    model_name: str
    datasets: list[ModelDataset]

    @property
    def path(self) -> str:
        return f"{self.parent}/{self.model_name}"
...
class SimulationResultsGroup(Group):
    ...
    def from_config(cls, config: Config, parent: RunGroup) -> Self:
        self = cls(parent)
        attr = SimulationResultsAttr.from_config(config[cls.CONFIG_KEY], self)
        self.attr = attr
        return self
    ...
    def from_hdf5(cls, hdf5: HDF5, parent: RunGroup) -> Self:
        self = cls(parent)
        time_series_dataset = TimeSeriesDataset.from_hdf5(hdf5, parent)
        self.time_series = time_series_dataset
        attr = SimulationResultsAttr.from_hdf5(hdf5, parent)
        self.attr = attr
        return self
...
class SimulationResultsAttr(Attribute):
    ...
    @classmethod
    def from_config(
        cls, config: SimulationConfig, parent: SimulationResultsGroup
    ) -> Self:
    ...
    def to_hdf5(self, hdf5: HDF5) -> None:
        hdf5.append_attributes(self.parent.path, self.to_dict())
...
class TimeSeriesDataset(Dataset):
    ...
    @property
    def path(self) -> str:
        return f"{self.parent}/{self.DATASET_NAME}"
    ...
    def to_hdf5(self, hdf5: HDF5) -> None:
        hdf5.store_data(
            self.data.to_records(index=False),
            data_name=self.name,
...
def init_hdf5(hdf5_path: Path) -> HDF5:
    ...
    hdf5.create_group("models")
    hdf5.create_group("models/fmus")
    hdf5.create_group("models/python_models")
    hdf5.create_group("models/fmu_origin")

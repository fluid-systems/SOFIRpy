...
class InitiateProject:
    ...
    def __init__(self, working_directory: str, hdf5_file_name: Optional[str] = None, pid_generator: Optional[Callable[[str], str]] = None) -> None:
        ...
        self.hdf5_path = os.path.normpath(os.path.join(self.working_directory, self.hdf5_file_name))
...
class Project(InitiateProject):
    ...
    def __init__(self, working_directory, hdf5_file_name = None, pid_generator = None, run_name_generator: Optional[Callable[[str, int], str]] = None) -> None:
    ...
    def create_run(self,hdf5_sub_groups: list[str], run_name: Optional[str] = None, attr = None) -> None:
    ...
    def check_run_exists(self) -> None:
        ...
        if exists_as_folder:
            if exists_in_hdf5:
                while True:
                    overwrite = input(f"The run '{self.current_run_name}' already exists in the hdf5 and as a folder in {self.working_directory}. Overwrite? [y/n]")
                    if overwrite == "y":
